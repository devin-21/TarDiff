    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:  # assume inverse normalization has been applied
            ...
            if len(grid.shape) == 3:
                ...
                if k == 'samples_swapping_intercept':
                    for i in range(n-1):  
                        grid[i] = dm.inverse_transform(grid[i], data_name=key_list[0])  # always the first one except for the last row (the second one)
                    grid[n-1] = dm.inverse_transform(grid[n-1], data_name=key_list[1])  # always the first one except for the last row (the second one)
                else:
                    for i in range(b):  
                        grid[:,i] = dm.inverse_transform(grid[:,i], data_name=key_list[i]) 
...
def prepare_trainer_configs(nowname, logdir, opt, lightning_config, ckptdir, model, now, cfgdir, config, trainer_opt):
    ...
    if lightning_config.trainer.grad_watch:
        ...
        if not opt.uncond:
            trainer_kwargs["logger"].watch(model.cond_stage_model, log='parameters', log_freq=1000)

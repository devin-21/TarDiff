...
class SplitTSEncoder(nn.Module):
    ...
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, dropout=0., emb_dropout=0., bn=True):
        super().__init__()
        ...
        for _ in range(num_layers):
            self.specific_encoder_layers.append(nn.ModuleList([
                PreNorm(dim_out, Attention(query_dim=self.latent_dim, context_dim=self.latent_dim, heads=num_heads, dim_head=dim_head), context_dim=self.latent_dim),
...
class OnlyPrototypeEncoder(nn.Module):
    ...
    The input are encoded into one embedding.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, dropout=0., emb_dropout=0., bn=True, orth_emb=False, mask_assign=False, hard_assign=False):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
    ...
    def forward(self, x):
        ...
        for attn, ff in self.specific_encoder_layers:
            sh = attn(sh, context=latents, mask=mask, hard_assign=self.hard_assign) + sh
            sh = ff(sh) + sh
...
class ProtoAssignEncoder(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, dropout=0., emb_dropout=0., bn=True, mask_assign=False, hard_assign=False, orth_emb=False):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
        self.mask_assign = mask_assign
...
class DomainUnifiedEncoder(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, split_inv=True, use_prototype=True, mask_assign=False, hard_assign=False, orth_proto=False, grad_hook=False, **kwargs):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
        self.mask_assign = mask_assign
        ...
        if self.split_inv:
            ...
            if self.use_prototype:
                # self.latents = nn.Parameter(torch.randn(num_latents, self.latent_dim), requires_grad=True)
                self.latents = nn.Parameter(torch.empty(num_latents, self.latent_dim), requires_grad=True)
...
class DomainUnifiedEncoderHook(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, split_inv=True, use_prototype=True, mask_assign=False, hard_assign=False, orth_proto=False, grad_hook=False):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
        self.mask_assign = mask_assign
...
class DomainProtoMaskEncoder(nn.Module):
    ...
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, split_inv=True, use_prototype=True, mask_assign=False, hard_assign=False, orth_proto=False, grad_hook=False):
        ...
        self.mask_ffn = nn.Sequential(
            ResBlockTime(dim, dim, bn=bn),
            View((-1, flatten_dim)),                  # batch_size x 2048
            nn.Linear(flatten_dim, dim_out),  # nn.Linear(flatten_dim, self.num_latents)
        )
        self.sigmoid = nn.Sigmoid()
        self.mask_attn_layer = PreNorm(dim_out, Attention(query_dim=self.latent_dim, context_dim=self.latent_dim, heads=num_heads, dim_head=dim_head), context_dim=self.latent_dim)
        self.mask_ff_layer = PreNorm(dim_out, nn.Linear(dim_out, self.num_latents))
...
class DomainUnifiedPrototyper(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, hard_assign=False, orth_proto=False, grad_hook=False, **kwargs):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
        self.hard_assign = hard_assign
...
class DomainEmbProtoMask(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, split_inv=False, hard_assign=False, orth_proto=False, grad_hook=False, **kwargs):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim
        self.hard_assign = hard_assign
...
class DomainEmbProtoAssignMask(nn.Module):
    ...
    The length of the two part are equal in this implementation.
    '''
    def __init__(self, dim, window, num_heads=1, num_layers=1, num_latents=16, num_channels=3, latent_dim=32, dim_head=64, bn=True, split_inv=False, mask_method='soft', orth_proto=False, grad_hook=False, **kwargs):
        super().__init__()
        self.num_latents = num_latents
        self.latent_dim = latent_dim

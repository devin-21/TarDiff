...
class GuideDDIMSampler(object):
    def __init__(self, model, schedule="linear", guide_type='l2', GDCalculater=None, **kwargs):
        super().__init__()
        self.model = model
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      unconditional_guidance_scale=1., unconditional_conditioning=None,label=None,**kwargs):
        ...
        iterator = tqdm(time_range, desc=f'DDIM Guided Sampler with dynamic scale', total=total_steps)
    ...
    def semantic_scoring(self, pred_x0, cond, label=None, t=None):
        ...
        if self.guide_type == 'cosine':
            if cond.dim() == 2:
                sim_score = torch.nn.functional.cosine_similarity(cond[:,-32:], pred_cond[:,-32:], dim=-1)
            else:  # assume the last condition is the target condition
                sim_score = torch.nn.functional.cosine_similarity(cond[:,-1], pred_cond[:,-1], dim=-1)
            scale = (1 / max(sim_score.mean(), 0.05)) ** 2
        elif self.guide_type == 'l2':
            if cond.dim() == 2:
                sim_score = -torch.nn.functional.pairwise_distance(cond[:,-32:], pred_cond[:,-32:], p=2)
            else:
                sim_score = -torch.nn.functional.pairwise_distance(cond[:,-1], pred_cond[:,-1], p=2)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in,label=label).chunk(2)

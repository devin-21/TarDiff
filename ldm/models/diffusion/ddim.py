...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        b = shape[0]
        label = torch.ones(b, dtype=torch.long, device=device) if label is None else torch.tensor(label, dtype=torch.long, device=device)
        if x_T is None:
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(x = img, c=cond, mask=mask, label=label, t=ts, index=index, use_original_steps=ddim_use_original_steps,
    ...
    @torch.no_grad()
    def p_sample_ddim(self, x, c, t, index, label=None,repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,

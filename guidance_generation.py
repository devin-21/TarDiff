...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-b","--base",nargs="*",metavar="base_config.yaml",help="paths to base configs. Loaded from left-to-right. "
    ...
    parser.add_argument("--normalize",type=str,const=True,default=None,nargs="?",help="normalization method",)
    ...
    parser.add_argument("-l","--logdir",type=str,default="/mnt/storage/ts_diff_newer",help="directory for logging dat shit",)
    ...
    parser.add_argument("-dw","--dis_weight",type=float,const=True,default=1.,nargs="?",help="weight of disentangling loss",)
    parser.add_argument("-dt","--dis_loss_type",type=str,const=True,default=None,nargs="?",help="type of disentangling loss",)
    parser.add_argument("-tg","--train_stage",type=str,const=True,default='pre',nargs="?",help="pre / dis",)
    parser.add_argument("-ds","--dataset_name",type=str,const=True,default='elec',nargs="?",help="dataset name",)
    parser.add_argument("-dp","--dataset_prefix",type=str,const=True,default='/mnt/storage/tsdiff/data',nargs="?",help="dataset prefix",)
    parser.add_argument("-cp","--ckpt_prefix",type=str,const=True,default='/mnt/storage/tsdiff/outputs',nargs="?",help="ckpt prefix",)
    parser.add_argument("-sp","--sample_path",type=str,const=True,default='/mnt/storage/ts_generated/ours_amlt',nargs="?",help="samples prefix",)
    
    parser.add_argument("-pl","--pair_loss_type",type=str, const=True, default='',nargs="?", help="pair loss type: cosine or l2, otherwise not used")
    ...
    parser.add_argument("-si","--split_inv", action='store_true', help="split invariant encoder")
    parser.add_argument("-cl","--ce_loss", action='store_true', help="cross entropy loss")
    parser.add_argument("-up","--use_prototype", action='store_true', help="use prototype")
    parser.add_argument("-pd","--part_drop", action='store_true', help="use partial dropout conditions")
    parser.add_argument("-o","--orth_emb", action='store_true', help="use orthogonal prototype embedding")
    parser.add_argument("-ma","--mask_assign", action='store_true', help="use mask assignment")
    parser.add_argument("-ha","--hard_assign", action='store_true', help="use hard assignment")
    parser.add_argument("-im","--inter_mask", action='store_true', help="use intermediate assignment")
    parser.add_argument("-bs","--batch_size", type=int, const=True, default=256,nargs="?", help="batch_size")
    parser.add_argument("-ms","--max_step_sum", type=int, const=True, default=20000,nargs="?", help="max training steps")
    parser.add_argument("-nl","--num_latents", type=int, const=True, default=16,nargs="?", help="sequence length")
    parser.add_argument("-pw","--pair_weight", type=float, const=True, default=1.0, nargs="?", help="pair loss weight")
...
if __name__ == "__main__":
    ...
    if opt.uncond:
        ...
        config.model['params']['cond_stage_config']['params']['split_inv'] = False
        config.model['params']['unet_config']['params']['latent_unit'] = opt.num_latents

        config.model['params']['cond_stage_config']['params']['use_prototype'] = False
        config.model['params']['cond_stage_config']['params']['mask_assign'] = False
        config.model['params']['cond_stage_config']['params']['hard_assign'] = False
        config.model['params']['unet_config']['params']['inter_mask'] = False
        config.model['params']['cond_stage_config']['params']['orth_proto'] = False
    ...
    gen_ckpt_path = '/home/v-dengbowen/code/dis-ts-diff/eval_model/622/MIMIC_LOS/last.ckpt'
    downstream_pth_path = '/home/v-dengbowen/code/TarDiff/classifier/checkpoints/best_model.pt'
    save_path_origin = '/home/v-dengbowen/mount/dataset/newer/mimic-iii/mimic-iii/readmission'
    origin_data_path = '/home/v-dengbowen/mount/dataset/newer/mimic-iii/mimic-iii/readmission/train_tuple.pkl'
    alpha = 0.00001
    save_path = origin_data_path.split('/')[:-1]
    save_path = save_path_origin

    train_tuple = pd.read_pickle(origin_data_path)

    generation_nums_label = dict(Counter(train_tuple[1]))

    synt_nums_label = dict(Counter(train_tuple[1]))
    config.model['params']['ckpt_path'] = gen_ckpt_path
    ...
    downstream_model.load_state_dict(torch.load(downstream_pth_path)['model_state'])
    print('#### Downstream Model Loaded #####')

    print("#### Start Generating Samples #####")
    for dataset in data.norm_train_dict:
        normalizer = data.normalizer_dict[dataset]
        train_dataset = TimeSeriesDataset(data.transform(train_tuple[0].transpose(0,2,1),normalizer), train_tuple[1],normalize=False)
        train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False)
        c = GradDotCalculator(downstream_model, train_dataloader , nn.BCEWithLogitsLoss(), alpha)
        ...
        for label_sample, total_samples in tqdm(generation_nums_label.items()):
            label = torch.tensor([label_sample]*total_samples, device='cuda', dtype=torch.long)
            samples, z_denoise_row = model.sample_log(cond=None, batch_size=total_samples, ddim=True, ddim_steps=20, eta=1., sem_guide=True, sem_guide_type='GDC',label=label, GDCalculater = c)
            ...
            if generated_data_all is None:
                ...
                generated_data_all = np.concatenate([generated_data_all, generated_data], axis=0)
        generated_data_all = generated_data_all.transpose(0, 2, 1)
        label = np.concatenate([np.full(count, label) for label, count in generation_nums_label.items()])
        tmp_name=f'synt_tardiff_noise_rnn_train_guidance_sc{alpha}'
        with open(save_path + '/' f'{tmp_name}.pkl', 'wb') as f:
